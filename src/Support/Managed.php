...
final class Managed
{
     ...
     */
    protected static $storageRoot = null;
    /**
    ...
    {
        empty(static::$hostName) && static::initialize();
    ...
    protected static function getClusterConfiguration($key = null, $default = null)
    {
        ...
        try {
            ...
            if (empty(static::$config) || !is_array(static::$config) || !static::validateConfiguration()) {
                return false;
            }
        } catch (\Exception $_ex) {
            static::$config = [];
    ...
    /**
     * Validates that the required values are in static::$config from .dfe.cluster.json
     *
     * @param bool $cached If true, deeper checking is done
     *
     * @return bool
     */
    protected static function validateConfiguration($cached = false)
    {
        try {
            $_url = static::getConfig('console-api-url');

            //  Can we build the API url
            if (empty($_url) || !static::getConfig('console-api-key')) {
                /** @noinspection PhpUndefinedMethodInspection */
            ...
            static::setConfig('console-api-url', rtrim($_url, '/ ') . '/');
            ...
            //  Cached data should have audit and environment
            if ($cached && (!static::getConfig('audit') || !static::getConfig('env'))) {
                return false;
            }
     ...
     */
    protected static function initializeAuditing()
    {
        ...
        if (!empty($_env) && isset($_env['audit-host'], $_env['audit-port'])) {
            ...
            !empty($_audit = static::getConfig('audit')) && Audit::setMetadata($_audit);
        }
    }
    ...
    protected static function loadCachedValues()
    {
        ...
        if (!empty($_cache) && is_array($_cache)) {
            ...
            static::$hostName = static::getConfig('managed.host-name');

            return static::validateConfiguration(true);
        }
    ...
    protected static function freshenCache()
    {
        ...
        /** @noinspection PhpUndefinedMethodInspection */
        Cache::put(static::$cacheKey, static::$config, static::CACHE_TTL);
    ...
    {
        return static::$cacheKey ?: static::$cacheKey = sha1(static::CACHE_KEY_PREFIX . static::getHostName());
    }
